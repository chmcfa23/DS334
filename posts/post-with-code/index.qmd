...
```{r}
library(dplyr)  
strep_tb <- strep_tb %>%   mutate(
  base_num = case_when(baseline_condition == "1_Good" ~ 3,              baseline_condition == "2_Fair" ~ 2,                              baseline_condition == "3_Poor" ~ 1)          
  ) 
```
...
ggplot(data = strep_tb, aes(x = rad_num)) + 
  ...
  labs(title = "TB Patient Recovery With and Without Streptomycin", x = "Radiologic Number", caption = "*Radiologic Number represents the degree to which patients recovered after the experiment, with 6 \n indicating 'Considerable Improvement,' and 1  indicating that the patient passed away") +   scale_x_continuous(breaks=seq(1,6,1)) + 
  facet_wrap(~arm)
```
